...
{
    public partial class SaveToExcel : Component
    {
        public SaveToExcel()
        {
            InitializeComponent();
        ...
        public SaveToExcel(IContainer container)
        {
            container.Add(this);
        ...
        public void Save(List<HeaderExcelModel> headers, List<object> objs, string name)
        {
            if (headers.Count < 1)
            {
                throw new Exception("Not headers");
            }

            using (SpreadsheetDocument spreadsheetDocument =
              SpreadsheetDocument.Create($"{name}.xls", SpreadsheetDocumentType.Workbook))
            {
               ...
               spreadsheetDocument.WorkbookPart.Workbook.AppendChild<Sheets>(new Sheets());
                ...
                for (int i = 0; i < headers.Count; i++)
                {
                    ...
                    InsertCellInWorksheet(new ExcelCellParameters
                    {
                        ...
                        ColumnName = $"{(char)(64 + Math.Max(1, headers[i].NumFirstColumnHeader))}",
                        RowIndex = 1,
                    ...
                    {
                        Worksheet = worksheetPart.Worksheet,
                        CellFromName = $"{(char)(64 + Math.Max(1, headers[i].NumFirstColumnHeader))}1",
                        CellToName = $"{(char)(64 + Math.Max(1, headers[i].NumLastColumnHeader))}1"
                    });
                }
                ...
                foreach (var field in objFirst.GetType().GetProperties())
                {
                    InsertCellInWorksheet(new ExcelCellParameters
                    {
                        Worksheet = worksheetPart.Worksheet,
                        ShareStringPart = shareStringPart,
                        ColumnName = "" + ((char)(numColumn + 64)),
                        RowIndex = rowIndex,
                        Text = field.Name,
                        StyleIndex = 2U
                    });
